...
int main(int argc, char** argv )
{
    ...
    std::string fileName("sysmemtester_kernel64.spv");
...

    {
        ...
        op.add<popl::Value<size_t>>("s", "size", "Size of Memory to Allocate", gwx, &gwx);
    ...
    CHECK_CALL( zeDeviceGetMemoryAccessProperties(hDevice, &memAccessProps) );

    if (memAccessProps.sharedSystemAllocCapabilities == 0) {
        printf("This device does not support shared system memory allocations?\n");
        return -1;
    }
...
#if 1
    std::unique_ptr<uint32_t[]> pDst = std::make_unique<uint32_t[]>(gwx);
#else
    uint32_t* pDst = nullptr;
    ze_host_mem_alloc_desc_t hostAllocDesc = {};
    hostAllocDesc.stype = ZE_STRUCTURE_TYPE_HOST_MEM_ALLOC_DESC;
    CHECK_CALL( zeMemAllocHost(context, &hostAllocDesc, gwx * sizeof(uint32_t), 0, (void**)&pDst) );
#endif
    ...
    if (gwx > 3) {
        printf("First few values:\n"
            ...
            pDst[0], pDst[0], *((float*)&pDst[0]),
            pDst[1], pDst[1], *((float*)&pDst[1]),
            pDst[2], pDst[2], *((float*)&pDst[2]));
    }
