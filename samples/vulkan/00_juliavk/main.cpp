...
static void PrintExternalMemoryTypeFlags(
    ze_external_memory_type_flags_t flags )
{
    #define CHECK_PRINT_FLAG(_e) if (flags & _e) printf("\t\t" #_e "\n");
    CHECK_PRINT_FLAG(ZE_EXTERNAL_MEMORY_TYPE_FLAG_OPAQUE_FD);
    CHECK_PRINT_FLAG(ZE_EXTERNAL_MEMORY_TYPE_FLAG_DMA_BUF);
    CHECK_PRINT_FLAG(ZE_EXTERNAL_MEMORY_TYPE_FLAG_OPAQUE_WIN32);
    CHECK_PRINT_FLAG(ZE_EXTERNAL_MEMORY_TYPE_FLAG_OPAQUE_WIN32_KMT);
    CHECK_PRINT_FLAG(ZE_EXTERNAL_MEMORY_TYPE_FLAG_D3D11_TEXTURE);
    CHECK_PRINT_FLAG(ZE_EXTERNAL_MEMORY_TYPE_FLAG_D3D11_TEXTURE_KMT);
    CHECK_PRINT_FLAG(ZE_EXTERNAL_MEMORY_TYPE_FLAG_D3D12_HEAP);
    CHECK_PRINT_FLAG(ZE_EXTERNAL_MEMORY_TYPE_FLAG_D3D12_RESOURCE);
    #undef CHECK_PRINT_FLAG

    ze_external_memory_type_flags_t extra = flags & ~(
    ...
    if (extra) {
        printf("\t\t(Unknown capability: %08" PRIx32 ")\n", extra);
    }
}
...
private:
    ...
    ze_device_handle_t deviceL0 = nullptr;
    ...
    void initL0() {
        ...
        deviceL0 = devices[deviceIndex];

        ze_device_properties_t deviceProps = {};
        deviceProps.stype = ZE_STRUCTURE_TYPE_DEVICE_PROPERTIES;
        CHECK_CALL( zeDeviceGetProperties(deviceL0, &deviceProps) );
        ...
        queueDesc.ordinal = 0;
        queueDesc.index = 0;
        CHECK_CALL( zeCommandListCreateImmediate(context, deviceL0, &queueDesc, &queue) );

        printf("Reading SPIR-V from file: %s\n", spv_filename);
        auto spirv = readFile(spv_filename);
        ...
        CHECK_CALL( zeModuleCreate(context, deviceL0, &moduleDesc, &module, &buildLog) );
    ...
    void createInstance() {
        ...
        appInfo.pApplicationName = "Julia Set Vulkan Sample";
